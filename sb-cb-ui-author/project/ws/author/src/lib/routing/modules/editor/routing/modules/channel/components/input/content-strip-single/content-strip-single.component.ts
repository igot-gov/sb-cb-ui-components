...
import { IPickerContentData } from '@ws-widget/collection'
...
export class ContentStripSingleComponent implements OnInit {
  @Output() data = new EventEmitter<{
    content: any // NsContentStripSingle.IContentStripSingle
    isValid: boolean
  }>()
  @Input() content!: any //  NsContentStripSingle.IContentStripSingle
  @Input() identifier = ''
  ...
  ngOnInit() {
    ...
    if (this.content.request) {
      if (this.content.request.ids && this.content.request.ids.length) {
        this.content.request.ids.map((v: any) =>
    ...
    if (
      ...
      this.content.request.manualData.map((v: any) => {
